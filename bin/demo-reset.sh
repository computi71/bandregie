...
# Quere kommen.
#
# Aufruf:   sh bin/demo-reset.sh [kennwort] [-v]
# Per cron: 5 * * * *  sh /pfad/zur/demo/bin/demo-reset.sh
#
# Bei Erfolg bleibt es still — deshalb braucht der cron-Eintrag auch keine
# Umleitung in eine Protokolldatei. Wer zusehen will, hängt -v an.
#
...
# Schalter und Kennwort auseinanderhalten, damit „demo-reset.sh -v" nicht das
# Kennwort auf „-v" setzt.
VERBOSE=""
KENNWORT=""
for arg in "$@"; do
  case "$arg" in
    ...
    *) [ -z "$KENNWORT" ] && KENNWORT="$arg" ;;
  esac
done

DEMO_PASSWORD=${KENNWORT:-${BANDREGIE_DEMO_PASSWORD:-demo}}
...
if command -v flock >/dev/null 2>&1; then
  exec flock -n "$LOCK" $RUN "$PHP" "$SCRIPT_DIR/demo-reset.php" "$DEMO_PASSWORD" $VERBOSE
fi

echo "Hinweis: flock nicht vorhanden — Reset läuft ohne Sperre." >&2
exec $RUN "$PHP" "$SCRIPT_DIR/demo-reset.php" "$DEMO_PASSWORD" $VERBOSE
