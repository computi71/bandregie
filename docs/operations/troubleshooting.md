...
Symptoms that have actually come up, with the cause behind each one. The order
is roughly "how often".

## The number on the app icon disappears

It can only change two ways: a push arrives, or the application is opened. If
neither happened, nothing repainted it — and iOS clears the badge on its own in
quiet stretches (a restart does it, and so does swiping the notifications away).
...
2. **Is the session still alive?** Without a session the app fetches no count,
   ...
   every use.
3. **Is the subscription alive?** A successful delivery stamps it. A subscription
   that has not been delivered to for a long time is usually a device whose push
   channel the platform has discarded — re-allow notifications on that device and
   a fresh channel is created.

A web application cannot refresh a badge in the background on iOS: every push
must show a visible notification, and silent refresh pushes are not allowed.
A daily reminder — sent only when something is actually open — is the honest
way to keep the number standing.
...
*Send access details* in the member list, which mints a new one.
...
## An administrator cannot see the treasury or the mailbox

By design. Two areas are never handed out by rank: running a band is not the
same as running its money or reading its post. They have to be granted per
account, in the member list — including to administrators, including to the one
who installed the thing.

A freshly created administrator therefore starts with neither. That is the point
of marking them explicit, and it is worth saying out loud when a new admin
joins.
...
[Running an instance](running-an-instance.md).
