...
$privacyDefault = "Datenschutzerklärung\n\n"
  ...
  . "6. Adress-Suche über OpenStreetMap (nur wenn aktiviert)\nWenn wir diese Funktion einschalten, wird beim Suchen einer Adresse die eingegebene Adresse von unserem Server an den Dienst Nominatim der OpenStreetMap Foundation (Vereinigtes Königreich) übermittelt, um die Koordinaten zu erhalten. Bleibt die Suche ohne Treffer, wiederholt der Server sie mit weniger Angaben — höchstens dreimal je Klick. Dein Browser nimmt daran nicht teil; deine IP-Adresse wird dabei nicht übermittelt. Zweck ist eine punktgenaue Navigation zu Veranstaltungsorten (Art. 6 Abs. 1 lit. f DSGVO). Ohne diese Funktion findet keine solche Übermittlung statt.\n\n"
  ...
  . "12. OneDrive (nur wenn verbunden)\nIst die Verbindung zu OneDrive eingerichtet, meldet sich unser Server mit einem von Microsoft ausgestellten Zeichen an. Anbieter ist die Microsoft Ireland Operations Ltd. (Irland), die dabei die IP-Adresse unseres Servers und den Zeitpunkt der Anfrage verarbeitet; dein Browser nimmt daran nicht teil, und deine IP-Adresse wird nicht übermittelt. Im Einzelnen: (a) Verknüpfte Ordner werden gelesen — empfangen werden Dateinamen, Ordnerpfade, Größen, Änderungszeitpunkte sowie die von Microsoft bereitgestellten Aufnahmedaten der Bilder (Aufnahmezeitpunkt, Kameramodell, Bildmaße, Prüfsumme und, sofern die Kamera sie aufgezeichnet hat, Ortskoordinaten). Diese Angaben speichern wir bei uns. (b) Übernehmen wir verknüpfte Bilder in die Galerie, laden und speichern wir eine von Microsoft gerechnete Vorschaufassung (etwa 800 Pixel); das Original bleibt bei OneDrive. Die Vorschaufassung enthält selbst keine Aufnahmedaten. (c) Ist der tägliche Abgleich eingeschaltet, verbindet sich unser Server einmal täglich ohne weiteres Zutun, um die verknüpften Ordner erneut zu lesen. (d) Ist OneDrive als Sicherungsziel eingerichtet, laden wir unsere Sicherungsarchive (Datenbank und hochgeladene Dateien, siehe Ziffer 11, verschlüsselt, sofern ein Schlüssel gesetzt ist) in einen Ordner des verbundenen Kontos hoch und löschen dort die ältesten, wenn die eingestellte Zahl überschritten ist; dafür ist neben dem Lese- auch ein Schreibrecht erteilt. Gespeichert werden bei uns der Name und die Adresse des verbundenen Microsoft-Kontos sowie die Zeichen für den Zugang, letztere verschlüsselt, sofern ein Schlüssel gesetzt ist. Rechtsgrundlage ist unser berechtigtes Interesse an einer Ablage ohne doppelte Datenhaltung und an einer Sicherung außerhalb des eigenen Servers (Art. 6 Abs. 1 lit. f DSGVO). Die Verbindung lässt sich in den Einstellungen jederzeit lösen; die Zeichen werden dann gelöscht, bereits übernommene Vorschaubilder und gespeicherte Angaben bleiben, bis sie dort entfernt werden. Die Zustimmung im Microsoft-Konto selbst ist dort zu entziehen.\n\n"
  . "13. Postfach der Band (nur wenn eingerichtet)\nIst ein Postfach eingerichtet, meldet sich unser Server regelmäßig beim Mailserver dieses Postfachs an und liest dessen Nachrichten. Übermittelt werden dabei die Zugangsdaten des Postfachs; empfangen werden Absender, Betreff, Sendezeitpunkt und der Textinhalt der Nachrichten, die wir bei uns speichern, um sie im Bandbereich anzuzeigen und daraus Termine anzulegen. Verarbeitet werden damit auch personenbezogene Daten der Absender. Gelesen wird nur; im Postfach selbst wird nichts verändert oder als gelesen markiert. Anhänge werden nicht übernommen. Beantworten wir eine Nachricht aus der Anwendung heraus, geht die Antwort über denselben Weg wie unsere übrigen E-Mails hinaus und wird bei uns gespeichert. Anbieter des Postfachs ist [Name und Anschrift des Mailanbieters]. Rechtsgrundlage ist unser berechtigtes Interesse an einer nachvollziehbaren Bearbeitung von Anfragen (Art. 6 Abs. 1 lit. f DSGVO), bei einer Anfrage zugleich die Anbahnung eines Vertrags (Art. 6 Abs. 1 lit. b DSGVO). Nachrichten lassen sich im Bandbereich ins Archiv legen; gelöscht werden sie dort, wo sie liegen — bei uns und im Postfach getrennt.\n\n"
  . "14. Gäste (nur wenn gebucht)\nBuchen wir für einen Auftritt Personen, die nicht zur Band gehören — etwa Tontechnik oder Aushilfsmusiker —, speichern wir Name, Aufgabe, E-Mail-Adresse, Telefonnummer und Notizen zur Zusammenarbeit sowie je Einsatz Zu- oder Absage und den Zeitpunkt. Die Einladung geht als E-Mail hinaus und enthält einen persönlichen Link; darüber sieht der Gast für diesen einen Termin den Ablauf, den Stagerider und die Setliste, lesend und bis zum Mittag des Folgetags. Rechtsgrundlage ist die Anbahnung und Durchführung des Engagements (Art. 6 Abs. 1 lit. b DSGVO). Die Kontaktdaten bleiben gespeichert, solange wir mit der Person zusammenarbeiten; auf Wunsch löschen wir sie.\n\n"
...
<details class="card acc" name="setacc">
  ...
  <?php endif; ?>
  <?php if (isset($backupRuns[0]) && ($backupRuns[0]['od_ok'] ?? null) !== null && !(int) $backupRuns[0]['od_ok']): ?>
    <p class="warn">⚠ <?= e(t('bk_warn_od')) ?> <?= e($backupRuns[0]['message']) ?></p>
  <?php endif; ?>
  ...
  <?php // OneDrive als Ziel (#50): dieselbe Verbindung wie fürs Verknüpfen,
        // aber mit Schreibrecht. Die Meldungen der Läufe sagen, ob es klappt. ?>
  <?php $bkOd = backup_od_config(); ?>
  <form method="post" action="/intern/einstellungen/backup-ziele" class="form-grid"><?= csrf_field() ?>
    <input type="hidden" name="_od_form" value="1">
    <label class="checkbox span2"><input type="checkbox" name="backup_od_enabled" value="1" <?= $bkOd['enabled'] ? 'checked' : '' ?>> ☁ <?= e(t('bk_od_enabled')) ?></label>
    <label><?= e(t('bk_od_dir')) ?><input name="backup_od_dir" value="<?= e($bkOd['dir']) ?>" placeholder="Sicherungen"></label>
    <label><?= e(t('bk_od_keep')) ?><input type="number" name="backup_od_keep" min="1" max="365" value="<?= (int) $bkOd['keep'] ?>"></label>
    <p class="muted small span2">🔑 <?= e(t('bk_od_note')) ?></p>
    <?php if (!od_enabled()): ?><p class="warn span2">⚠ <?= e(t('od_needs_enable')) ?></p><?php endif; ?>
    <div class="span2 row-buttons"><button class="btn btn-primary"><?= e(t('save')) ?></button></div>
  </form>
  <form method="post" action="/intern/backup/od-test" class="inline"><?= csrf_field() ?>
    <button class="btn"><?= e(t('bk_od_test')) ?></button>
  </form>
