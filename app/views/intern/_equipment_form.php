 ...
 * und $items. Wird an zwei Stellen gebraucht: aufgeklappt bei eigenständigen
 * Geräten und im Dialog bei Bestandteilen.
 */
?>
<form method="post" action="/intern/equipment/<?= $formEq['id'] ?>/update" class="form-grid"><?= csrf_field() ?>
  ...
  <label data-eqinherit><?= e(t('eq_owner')) ?>
    <select name="owner_id"><option value=""><?= e(t('eq_owner_band')) ?></option><?php foreach ($members as $m): ?><option value="<?= $m['id'] ?>" <?= (int) $formEq['owner_id'] === (int) $m['id'] ? 'selected' : '' ?>><?= e($m['name']) ?></option><?php endforeach; ?></select>
  </label>
  ...
  <label><?= e(t('eq_purchased')) ?><input type="date" name="purchased_on" value="<?= e($formEq['purchased_on'] ?? '') ?>"></label>
  <label><?= e(t('eq_price')) ?><input type="number" name="price" step="0.01" min="0" value="<?= $formEq['price_cents'] !== null ? e(number_format((int) $formEq['price_cents'] / 100, 2, '.', '')) : '' ?>"></label>
...
<form method="post" action="/intern/equipment/<?= $formEq['id'] ?>/delete" onsubmit="return confirm('<?= e(t('confirm_delete')) ?>')" class="inline"><?= csrf_field() ?>
